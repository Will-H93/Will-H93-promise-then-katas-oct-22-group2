...
const cat = async () => {
    const catsUrl = await fetch('cats')
    return catsUrl.data.cats.sort();
};
...
const dog = async () => {
    const dogData = await fetch('dogs')
    const filterData = dogData.data.dogs.filter(dog => dog.naughty >= 10);
    return filterData[0]
};
